"""Company Root Supervisor Agent & Conversational Session Scaffold.
...
class StateSession:
    ...
    def update_from_prompt(self, prompt: Any) -> None:
        ...
        m_disc = re.search(r'(\d{1,3})\s*%', prompt_lower)
        if m_disc:
            try:
                self.set("session.discount_percentage", int(m_disc.group(1)))
            except ValueError:
                pass
...
class Company_Supervisor_Agent:
    """Root Supervisor Agent for European Camping Company (Company).
    ...
    def __init__(self, model_name: str = MODEL_SUPERVISOR):
        ...
        self.name = "Company_Supervisor_Agent"
        self.system_instruction = (
            "You are the Company Root Supervisor Agent. "
            "CRITICAL GOVERNANCE RULE: Whenever you invoke a state-changing tool or action that requires Human-in-the-Loop confirmation "
            "(such as staging CRM flash campaigns or updating PMS inventory), you MUST generate a clear, concise introductory text summary "
            "accompanying the tool call. Never emit an empty text response when calling confirmation-gated actions."
        )
    ...
    def process_turn(
        ...
        with tracer.span("Supervisor.process_turn", attributes={"session_id": session.session_id, "confirmed": confirmed}) as span:
            ...
            if pending and not confirmed:
                ...
                is_approval = any(re.search(r'\b' + kw + r'\b', prompt_lower) for kw in ["approve", "yes", "confirm", "ok", "oui", "valider", "ask this agent", "ask agent", "execute", "proceed"]) or "ask this agent" in prompt_lower
                if is_approval:
            ...
            if intent == "YIELD_ANALYTICS":
                ...
                is_readonly_pms = (intent == "PMS_OPERATIONS" and any(kw in prompt_lower for kw in ["retrieve", "distribution", "view", "show", "list", "get", "tickets", "ticket"]))
                if not confirmed and not is_readonly_pms:
