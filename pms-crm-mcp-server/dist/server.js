...
export async function loadCampsitesFromFirestore(campsiteIdFilter) {
    ...
    if (campsiteIdFilter) {
        return campsites.filter((c) => c.campsiteId === campsiteIdFilter);
    }
    return campsites;
}
export async function updateFirestoreUnits(campsiteId, unitIds, newStatus) {
    // Update in-memory cache
    const campsite = inMemoryCampsites.find((c) => c.campsiteId === campsiteId);
    if (campsite) {
...
export async function saveFirestoreCampaign(campaignData) {
...
export async function saveFirestoreTicket(ticketData) {
...
export async function loadTicketsFromFirestore(statusFilter, campsiteIdFilter) {
    if (firestoreDb) {
        try {
...
export async function updateFirestoreTicketStatus(ticketId, newStatus) {
...
export function createServer() {
    ...
    registerAppTool(server, "get-pms-inventory", {
        ...
        const summaryText = campsites
            .map((c) => `Campsite: ${c.name} (${c.campsiteId})\nTotal Units: ${c.units.length}\nHeld Back: ${c.units.filter((u) => u.status === "HELD_BACK").length}`)
            .join("\n\n");
        return {
            content: [
                {
    ...
    registerAppTool(server, "resalys-update-inventory", {
        ...
        const campsites = await loadCampsitesFromFirestore(args.campsiteId);
        const campsite = campsites.find((c) => c.campsiteId === args.campsiteId);
        const response = {
            status: "SUCCESS",
