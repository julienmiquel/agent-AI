...
import { createServer, loadCampsitesFromFirestore, updateFirestoreUnits, saveFirestoreCampaign, loadCampaignsFromFirestore, loadTicketsFromFirestore, saveFirestoreTicket, updateFirestoreTicketStatus, } from "./server.js";
...
export async function startStreamableHTTPServer(createServer) {
    ...
    app.use((_req, res, next) => {
        res.setHeader("Content-Security-Policy", "default-src 'self' 'unsafe-inline' 'unsafe-eval' data: blob: https:; connect-src 'self' https://ecg-pms-crm-mcp-app-ahavst3hhq-uc.a.run.app https://ecg-pms-crm-mcp-app-1008225662928.us-central1.run.app https://*.run.app https://*.googleapis.com;");
        next();
    });
    ...
    const serveHtml = (filename) => async (_req, res) => {
    ...
    apiRouter.get("/pms-inventory", async (req, res) => {
        try {
            const campsiteId = (req.query.campsiteId || req.query.campsite_id);
    ...
    apiRouter.get("/crm-campaigns", async (_req, res) => {
        try {
    ...
    apiRouter.get("/support-tickets", async (req, res) => {
        try {
            ...
            const campsiteId = req.query.campsiteId;
    ...
    apiRouter.post("/support-tickets/update-status", async (req, res) => {
        try {
    ...
    apiRouter.post("/resalys-update-inventory", async (req, res) => {
        try {
            const body = req.body || {};
    ...
    apiRouter.post("/crm-stage-flash-campaign", async (req, res) => {
        try {
            const body = req.body || {};
