-- European Camping Company (Company) BigQuery Seed Script
-- Dataset: company_analytics
-- Target Project: company-campsite-prod (or overridable via GCP_PROJECT_ID)

CREATE SCHEMA IF NOT EXISTS `company_analytics`;

-- 1. Table: occupancy_daily
CREATE TABLE IF NOT EXISTS `company_analytics.occupancy_daily` (
...
TRUNCATE TABLE `company_analytics.occupancy_daily`;
...
INSERT INTO `company_analytics.occupancy_daily` (cluster_id, campsite_id, date, occupied_units, total_capacity, total_revenue, nights_sold)
SELECT
...
CREATE TABLE IF NOT EXISTS `company_analytics.booking_segments` (
  cluster_id STRING NOT NULL,
  campsite_id STRING,
  date DATE,
  segment STRING,
...
TRUNCATE TABLE `company_analytics.booking_segments`;

-- Segment Performance Data (Dutch NL Market 15% lag in Med South)
INSERT INTO `company_analytics.booking_segments` (cluster_id, campsite_id, date, segment, target_units, booked_units, unit_id, status)
...
INSERT INTO `company_analytics.booking_segments` (cluster_id, campsite_id, date, segment, target_units, booked_units, unit_id, status) VALUES
