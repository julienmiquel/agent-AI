    ...
    for regex in COMPILED_INJECTION_REGEX:
        ...
        if match:
            ...
            return {
                ...
                "recovery_instruction": "Your prompt was flagged by our automated security guardrails for containing restricted keywords or instructions. Please reformulate your request to focus on Company Yield, PMS, or Marketing operations.",
            }
...
def check_business_guardrail(intent: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if intent == "MARKETING_CAMPAIGN" or "discount_percentage" in parameters or "session.discount_percentage" in parameters:
        disc = parameters.get("discount_percentage") if parameters.get("discount_percentage") is not None else parameters.get("session.discount_percentage")
        if disc is not None:
            try:
                disc_val = int(disc)
                if disc_val > 50:
    ...
    if "campsite_id" in parameters and parameters["campsite_id"]:
        ...
        if not any(cid.startswith(p) for p in valid_prefixes):
            violation = f"Campsite ID '{cid}' does not match any recognized Company property prefix."
            return {
                "passed": False,
                "violation": violation,
                "status": "VALIDATION_ERROR",
